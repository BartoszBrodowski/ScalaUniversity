...
import jp1.akka.lab13.model.Zawodnik.Próba
...
class Grupa(zawodnicy: List[ActorRef]) extends Actor {
  def receive: Receive = {
    case Grupa.Runda => {
      ...
      context.become(przyjmijWyniki(sender()))
      zawodnicy.foreach((zawodnik) => {
        zawodnik ! Próba
      })
    }
  }
  def przyjmijWyniki(organizator: ActorRef): Receive = {
    case Grupa.Wynik(ocena) =>
      val wynikZawodnika = (sender(), ocena)
      organizator ! wynikZawodnika
    // sender() === zawodnik
  }
}
